...
{
  const auto b0 = detail::byte_buffer(1);
...
TEST_CASE("A zero_byte_buffer is aligned on 16 bytes")
{
  const auto b0 = detail::zero_byte_buffer(1);
  REQUIRE((reinterpret_cast<std::size_t>(&b0[0]) % 16ul) == 0ul);
...
TEST_CASE("A byte_buffer is not 0-out when resized")
{
...
TEST_CASE("A zero_byte_buffer is 0-out when resized")
{
